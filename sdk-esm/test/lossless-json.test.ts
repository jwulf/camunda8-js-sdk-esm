...
Deno.test("LosslessJsonParser correctly handles nested Dtos", () => {
...
Deno.test("LosslessJsonParser can handle an array", () => {
	class ProcessInstanceStatistics extends LosslessDto {
...
Deno.test("LosslessJsonParser will throw when passed an unsafe int64 number and no Dto", () => {
	let threw = false;
...
Deno.test("LosslessJsonParser will throw when passed an unsafe int64 number and a Dto without a mapping for it", () => {
	let threw = false;
	const text = '{"normal":2.3,"long":123456789012345678901,"big":2.3e+500}';
	try {
...
Deno.test("LosslessJsonParser will parse a BigInt when passed an int64 number and a mapped Dto", () => {
	const text = '{"normal":2.3,"long":123456789012345678901,"big":2.3e+500}';
...
Deno.test("LosslessJsonParser with safe numbers and no Dto works like JSON.parse", () => {
	const json = `[
			{
				"activityId": "activityId",
				"active": 1,
...
Deno.test("LosslessJsonParser parses unexpected numbers as number at runtime", () => {
...
Deno.test("LosslessJsonParser is ok with missing optional fields at runtime", () => {
	class InputVariables extends LosslessDto {
		name!: string;
		@Int64String
		key!: string;
...
Deno.test("LosslessJsonParser will throw if an unexpected type is encountered at runtime", () => {
	class InputVariables extends LosslessDto {
		name!: string;
		@Int64String
		key!: string;
...
Deno.test("LosslessJsonParser throws for unexpected unsafe numbers at runtime", () => {
	class InputVariables extends LosslessDto {
		name!: string;
		@Int64String
		key!: string;
...
Deno.test("LosslessJsonParser/Stringify correctly handles array of nested Dtos", () => {
	class DecisionInstanceOutput extends LosslessDto {
		public someInt32Field!: number;
		@Int64String
	...
	) as unknown as ProcessDefinitionDto[];
	...
	expect(parsed[0].total).toBe(3)
	expect(parsed[0].decisionsOutputs[0].someInt64Field).toBe(123)
});

Deno.test("LosslessStringify parses safe unexpected numbers as number at runtime", () => {
	class InputVariables extends LosslessDto {
		name!: string;
		@Int64String
		key!: string;
	...
	expect(stringified).toBe(
		`{"name":"John Doe","key":12345678901234567890,"bigInt":12345678901234567890,"age":42}`,
	);
});

Deno.test("LosslessStringify correctly handles nested Dtos", () => {
	class DecisionInstanceOutput extends LosslessDto {
		public someInt32Field!: number;
		@Int64String
		public someInt64Field!: string;
	...
	expect(parsedDto.total).toBe("3");
	const stringified = losslessStringify(parsedDto);
	expect(typeof stringified).toBe("string");
	expect(stringified).toBe(
		`{"decisionsOutputs":[{"someInt32Field":123,"someInt64Field":123}],"total":3}`,
	);
});

Deno.test("LosslessJsonParser correctly handles null objects", () => {
	const json = `{"abc": [null, null, null] }`;

	const parsedDto = losslessParse(json);
	expect(parsedDto.abc[0]).toBeNull(); // 3 (string)
	expect(parsedDto.abc[1]).toBeNull(); // 3 (string)
	expect(parsedDto.abc[2]).toBeNull(); // 3 (string)
});

Deno.test("LosslessStringify correctly handles null objects", () => {
	const json = { abc: [null, null, null] };
...
Deno.test("LosslessJsonParser will throw if given stringified JSON with an unsafe integer number, even with a Dto", () => {
	let threw = false;
	const json = `{"unsafeNumber": 9223372036854775808}`; // Unsafe integer (greater than Int64 max)
...
Deno.test("It rejects Date, Map, and Set types", () => {
	class Dto extends LosslessDto {
		date?: Date;
		name?: string;
...
Deno.test("It correctly handles a number array in a subkey", () => {
	const json =
		`{"message":"Hello from automation","userId":null,"sendTo":[12022907,12022896,12022831]}`;
	const res = losslessParse(json);
	expect(res.sendTo[0]).toBe(12022907);
});

Deno.test("It correctly handles a number array in a subkey with a DTO (Int64StringArray)", () => {
	class Dto extends LosslessDto {
		message!: string;
		userId!: number;
		@Int64StringArray
...
Deno.test("It correctly handles a number array in a subkey with a DTO (BigIntValueArray)", () => {
	class Dto extends LosslessDto {
		message!: string;
		userId!: number;
		@BigIntValueArray
...
Deno.test("It correctly throws when encountering a number rather than an array in a subkey with a DTO (Int64StringArray)", () => {
	class Dto extends LosslessDto {
		message!: string;
		userId!: number;
		@Int64StringArray
	...
	const json =
		`{"message":"Hello from automation","userId":null,"sendTo":12022907}`;
	expect(() => losslessParse(json, Dto)).toThrow("expected Array");
});

Deno.test("It correctly throws when encountering a number rather than an array in a subkey with a DTO (BigIntValueArray)", () => {
	class Dto extends LosslessDto {
		message!: string;
		userId!: number;
		@BigIntValueArray
