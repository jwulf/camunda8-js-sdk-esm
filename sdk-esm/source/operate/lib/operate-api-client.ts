...
export class OperateApiClient {
	...
	constructor({ config, oAuthProvider, rest = ky }: OperateClientOptions = {}) {
		const configuration = isoSdkEnvironmentConfigurator
			.mergeConfigWithEnvironment(config);
	 ...
	 * @throws {RESTError}
	 */
	public async getJsonVariablesforProcess<
		T extends Record<string, JsonDocument>,
	>(
		processInstanceKey: number | string,
	): Promise<T> {
		const body = {
		...
		const entries = variables.items.map<[string, JsonDocument]>(
			(current) => [
				current.name,
				this.safeJsonParse<JsonDocument>(current.value),
			],
		);
